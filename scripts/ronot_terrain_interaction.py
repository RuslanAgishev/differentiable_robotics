...
def force(x, v, m, x_grid, y_grid, z_grid):
    # gravity force
    F_g = torch.tensor([0.0, 0.0, -m * g])
    ...
    # calculate force: underdamped spring-damper system
    k = 100.0
    omega0 = torch.sqrt(torch.tensor(k / m))
    d = 0.5 * omega0
    F_t = -(k * dh + d * v) * n * in_contact
    F = F_g + F_t
...
def simulate(x, xd, x_grid, y_grid, z_grid):
    ...
    for i in range(int(T / dt)):
        F = force(x, xd, m, x_grid, y_grid, z_grid)
        xdd = F / m
...
def main():
    ...
    for i in range(int(T / dt)):
        ...
        if i % 1 == 0:
            ...
            # plot total force
            ax.quiver(x[0].item(), x[1].item(), x[2].item(), F[0].item(), F[1].item(), F[2].item(), color='m')
            plt.pause(0.01)
